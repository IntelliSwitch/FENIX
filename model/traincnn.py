...
def main():
    parser = argparse.ArgumentParser(description='TextCNN1 Traffic Classifier')
    parser.add_argument('--dataset_name', required=True, choices=['iscxvpn', 'ustc-tfc2016'], help='Dataset name')
    parser.add_argument('--save_dir', default='./save/cnn', help='Model save directory')
    parser.add_argument('--log_dir', default='./log/train/cnn', help='Log save directory')
    ...
    parser.add_argument('--len_vocab', type=int, default=DEFAULT_LEN_VOCAB)
    parser.add_argument('--ipd_vocab', type=int, default=DEFAULT_IPD_VOCAB)
    parser.add_argument('--len_embedding_bits', type=int, default=DEFAULT_LEN_EMBEDDING_BITS)
    parser.add_argument('--ipd_embedding_bits', type=int, default=DEFAULT_IPD_EMBEDDING_BITS)
    parser.add_argument('--nk', type=int, default=DEFAULT_NK)
    parser.add_argument('--ebdin', type=int, default=DEFAULT_EBDIN)
    ...
    train_dataset = TrafficSequenceDatasetFromNPY(data_file=train_data_path, labels_file=train_labels_path,
                                                 sequence_length=args.window_size)
    test_dataset = TrafficSequenceDatasetFromNPY(data_file=test_data_path, labels_file=test_labels_path,
                                                sequence_length=args.window_size)
    ...
    model = TextCNN1(input_size=args.input_size, num_classes=num_classes,
    ...
    for epoch in range(args.epochs):
        train_loss, train_acc, _, _, train_f1 = train_epoch(model, train_loader, criterion, optimizer, DEVICE)
        val_loss, val_acc, _, _, val_f1 = validate_epoch(model, test_loader, criterion, DEVICE)
...
'''
python traincnn.py --dataset_name iscxvpn
python traincnn.py --dataset_name ustc-tfc2016
'''