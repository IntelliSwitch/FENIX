...
def evaluate_model_single_pass(onnx_model_path, data_npy_path, labels_npy_path, filenames_json_path, window_size, num_classes, batch_size=1, logger=None):
    ...
    for stream_key, window_indices in stream_data_dict.items():
        if window_indices:
            # Assume label is consistent within a stream; take the first one
        ...
        # Filter out failed predictions (-1) for voting
    ...
    # Filter out streams where prediction failed
...
def setup_logging(dataset_name, log_dir="./logs", log_filename=None):
    ...
    # --- Modification 1: Change log filename format ---
    if log_filename is None:
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        # Use the dataset name in the log filename
...
if __name__ == "__main__":
    # --- Modification 2a: Add argument for custom ONNX path ---
    ...
    parser.add_argument('--onnx_model_path', type=str, default=None, help='Path to the ONNX model file. If not provided, a default path based on dataset_name will be used.')
    
    args = parser.parse_args()
    DATASET_NAME_INPUT = args.dataset_name
    CUSTOM_ONNX_PATH = args.onnx_model_path # Get the custom path (could be None)

    QUANTIZE_MODEL_BASE_DIR = "./quantize_model/cnn"
    ...
    # --- Modification 2b: Determine ONNX path logic ---
    if CUSTOM_ONNX_PATH:
        # If custom path is provided, use it
        ONNX_MODEL_PATH = CUSTOM_ONNX_PATH
        print(f"Using custom ONNX model path: {ONNX_MODEL_PATH}")
    else:
        # If no custom path, use the default logic
        ONNX_MODEL_PATH = os.path.join(QUANTIZE_MODEL_BASE_DIR, DATASET_NAME_INPUT, "TextCNN1_int.onnx")
    ...
    if missing_paths:
         print(f"Error: The following required files/directories do not exist: {missing_paths}")
         exit(1) # Exit if files are missing

    BATCH_SIZE = 1

    # --- Modification 1: Pass dataset name to setup_logging ---
    logger, log_filepath = setup_logging(dataset_name=DATASET_NAME_INPUT, log_dir="./log/onnx/cnn", log_filename=None) 
    ...
    print("\n" + "="*50)
    print("Starting single-pass evaluation")
    print("="*50)
    print(f"Selected dataset: {DATASET_NAME_INPUT}")
    print(f"ONNX Model Path: {ONNX_MODEL_PATH}") # Print the actual path being used
    ...
    try:
        # Check if test files exist (already checked earlier, but good practice)
        if os.path.exists(TEST_DATA_NPY_PATH) and os.path.exists(TEST_LABELS_NPY_PATH) and os.path.exists(TEST_FILENAMES_JSON_PATH):
            ...
            results = evaluate_model_single_pass(
                onnx_model_path=ONNX_MODEL_PATH, # Use the determined path
    ...
    logger.info(f"ONNX model evaluation completed. Log file: {log_filepath}")
    print(f"\nONNX model evaluation completed. Log file: {log_filepath}")
